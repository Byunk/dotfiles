...
- Code links: prefer full GitHub `blob/<sha>/path#Lx-Ly` links for important ranges. GitHub renders linked code snippets in the PR, which makes reviews easier. Keep each linked range minimal: include only the lines needed to support the surrounding claim, never a broad implementation block. For a Rust type, link its declaration and only the relevant methods; use separate links when those ranges are not adjacent. Example: `https://github.com/a4s-lab/spreadsheet/blob/44aa272358fa048cfe2c52b0c6dccdb182513bc2/crates/workbook/src/operation.rs#L357-L370`.
...
- Link important code ranges with full GitHub URLs so GitHub shows code snippets inline. Keep every range as small as possible and exclude unrelated fields, methods, and surrounding code. For a Rust struct, link the struct declaration and only the minimal methods needed to explain the change, using separate links when necessary.
